...
- For executable dashboard requests, use the folder's `LIVE-DASHBOARD.json` / `LIVE-DASHBOARD.ipynb` and `tools/live_dashboard_runner.py`. Each folder has a committed dry-run `live-output/index.html`; real customer runs should go to ignored `live-dashboard-output/` or `cases/` folders.
